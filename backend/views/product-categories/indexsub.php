...
$this->title = Yii::t('yii', 'Product Sub Categories');
$this->params['breadcrumbs'][] = ['label' => Yii::t('yii', 'Product Categories'), 'url' => ['index']];
$clickCatID = Yii::$app->session->get('currentProdCatId');
$getNameOFCatForBD = \backend\models\ProductCategories::find()->where("category_id = $clickCatID")->one();
...
if(count($explodePath) > 0)
{
    ...
    {
        $catData = \backend\models\ProductCategories::find()->where("category_id = $vl")->one();
        if(count($explodePath) == $i){ 
        $this->params['breadcrumbs'][] = $catData->name;
        } 
        else { //$url = ['product-categories/indexsubcategory/'.$catData->category_id];   
            $url = ['product-categories/indexsubcategory','id'=>$catData->category_id];  
...
$this->params['menu'][] = ['label'=>'Delete Sub Category','url'=>['delete'],'options'=>['class'=>'widemenu']];
...
<input type="hidden" id="edit-url" value="<?php echo  Yii::$app->getUrlManager()->createUrl('product-categories/update'); ?>" >
                    ...
                    [
                                ...
                                'value' => function ($data) {
                                    ...
                                    return "<div class='dropdown'>
                                        ". Html::a('<span class="glyphicon glyphicon-trash" data-original-title="" title=""></span>',['product-categories/delete', 'id' =>$data->category_id],['data-confirm'=>'Are you sure to delete this item?','class'=>'quickactiondelete']) .
                         Html::a('<span class="glyphicon glyphicon-pencil" data-original-title="" title=""></span>',['product-categories/update', 'id' =>$data->category_id]).
                                            "</div>";
                                }
                            ],
                    //'category_id',
                        ...
                        'value' => function($model)
                            {
                                ...
                                {
                                    $url = Yii::$app->urlManager->createUrl(['product-categories/indexsubcategory', 'id' => $model->category_id]);
    ...
    $(document).ready(function () {
     // -- Start Key up on customer
         $('#genericSearch').keyup(function(){
                   ...
                   $.ajax({
                                     url:'<?php echo Yii::$app->urlManager->createUrl(['product-categories/getcategories']); ?>',
                                     type:"POST",
                                    dataType:"JSON",
                                     ...
                                     success:function (output) {
                                         ...
                                         $( "#genericSearch" ).autocomplete({
                                              select: function(event, ui) {
                                                    ...
                                                    var urlViewDetail = '<?php echo Yii::$app->urlManager->createUrl(['product-categories/indexsubcategory']); ?>';
