...
$currentCategory = Yii::$app->session->get('currentCatId');
...
<div class="categories-form">
    ...
    <?php $form = ActiveForm::begin(['options' => ['enctype'=>'multipart/form-data']]); ?>
     <?php 
            ...
            {
            $categoryAll = Categories::find()->where(['flagdelete'=>'0','status'=>'1','parent_id'=>$currentCategory])->all();
            $categoruData = ArrayHelper::map($categoryAll,'category_id','name');
            echo $form->field($model, 'type')->radioList([ 'C' => 'Course', 'W' => 'Webinar']);
            ...
            else
            {
                ...
                $categoryAll = Categories::find()->where(['flagdelete'=>'0','status'=>'1','category_id'=>$currentCategory])->all();
                else{
                $check = Categories::find()->where("category_id =  $currentCategory")->one(); //$model->checkHasSubCatOrNot($currentCategory);    
                  if($check->parent_id == 0)  
                    $categoryAll = Categories::find()->where(['flagdelete'=>'0','status'=>'1','parent_id'=>0])->all();
                  else
                    $categoryAll = Categories::find()->where(['flagdelete'=>'0','status'=>'1','parent_id'=>$check->parent_id])->all();  
                }
